...
def check_container_stats(container_name,collect_item):
    ...
    if collect_item == 'cpu_total_usage':
        ...
        result=float(result)/int(1024)/int(1024)
        ...
        network_check_command="""/usr/bin/docker exec %s cat /proc/net/dev|sed -n 3p|awk '{print $2,$10}'"""%container_name
        result=os.popen(network_check_command).read().split()[0]
        result=float(result)/int(1024)/int(1024)/int(1024)
