...
ajp_bio(){
  ajp_bio=`$java/java -jar $jmxclient/cmdline-jmxclient-0.10.3.jar - 127.0.0.1:$tomcat_port|grep 'ajp-bio'|awk -F '"' '{print $2}'|head -1`
}
#获取ajp-bio的端口号

http_bio(){
  http_bio=`$java/java -jar $jmxclient/cmdline-jmxclient-0.10.3.jar - 127.0.0.1:$tomcat_port|grep 'http-bio'|awk -F '"' '{print $2}'|head -1`
}
#获取http-bio的端口号
